  ...
  function explore(obj, path = '') {
    if (Array.isArray(obj)) {
      list.push({
        path: path || 'racine',
        type: 'Array',
        preview: `[Tableau de ${obj.length} éléments]`
      });
      if (obj.length > 0) {
        explore(obj[0], path ? `${path}[0]` : '[0]');
      }
    } else if (typeof obj === 'object' && obj !== null) {
      for (const key in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, key)) {
          const nextPath = path ? `${path}.${key}` : key;
          explore(obj[key], nextPath);
        }
      }
    } else {
      list.push({
...
<template>
  <div class="datavalue-picker-wrapper mb-3">
    ...
    <!-- Zone d'exploration des valeurs absolues -->
    <div v-if="isOpen" class="card mt-2 border-primary-subtle bg-body-tertiary shadow-sm transition-all duration-200">
      <div class="card-body p-3">
        ...
        <div v-else class="d-flex flex-column gap-3">
          ...
          <!-- Liste des valeurs et chemins détectés -->
          <div v-if="fetchedData && !isDataLoading">
            ...
            <div v-if="foundPathsList.length > 0" class="d-flex flex-column gap-2" style="max-height: 350px; overflow-y: auto;">
              <button 
                v-for="arr in foundPathsList" 
                :key="arr.path" 
                type="button" 
              ...
              >
                <div class="d-flex align-items-center gap-2 min-w-0">
                  <i class="bi text-primary" :class="arr.type === 'Array' ? 'bi-list-ul' : 'bi-hash'"></i>
                  <span class="font-monospace text-truncate fw-semibold">{{ arr.path }}</span>
                </div>
                <span class="text-secondary small text-truncate ms-2" style="max-width: 50%;">
                  {{ arr.preview }}
                </span>
              </button>
            </div>
