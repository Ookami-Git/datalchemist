...
const getSourceIcon = (src) => {
  switch (src) {
    case 'file': return 'bi-file-earmark-code';
    case 'url': return 'bi-link-45deg';
    case 'database': return 'bi-database';
    case 'text': return 'bi-blockquote-left';
        ...
        <div class="card-body p-3 p-lg-4 d-flex flex-column gap-3">
          <div class="d-flex flex-wrap align-items-center gap-3">
            <div class="admin-edit-source-hero-icon">
              <i class="bi" :class="getSourceIcon(JsonSource.src)"></i>
            </div>
            <div class="admin-edit-source-title-wrap me-auto">
              <p class="admin-edit-source-kicker mb-0">{{ $t('menu.edit') }}</p>
              <h5 class="mb-0 fw-bold text-gradient">{{ SourceInfo ? SourceInfo.name : $t('editsource.header') }}</h5>
              <p class="mb-0 small text-secondary">{{ $t('editsource.subtitle') }}</p>
            </div>
            <div class="d-flex align-items-center gap-2">
              <span v-if="SourceInfo" class="badge rounded-pill admin-edit-source-state-chip text-bg-light border-subtle">
          ...
          <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 pt-2 border-top border-subtle">
            <div class="d-flex align-items-center gap-2 flex-shrink-0">
              <RouterLink type="button" class="btn btn-light btn-sm px-3 rounded-pill" :to="{ name: 'edit', query: { tab: 'sources' } }" active-class="active">
                <i class="bi bi-arrow-left me-1"></i>{{ $t('menu.edit') }}
              </RouterLink>
              <button v-if="SourceInfo" type="button" class="btn btn-outline-primary btn-sm px-3 rounded-pill"
                :title="$t('global.preview_saved_hint')" @click="isPreviewOpen = true">
                <i class="bi bi-eye-fill me-1"></i>{{ $t('global.preview') }}
              </button>
            </div>

            <div class="flex-grow-1 max-w-md-50" v-if="SourceInfo">
              <div class="input-group input-group-sm admin-edit-source-name-input-modern">
        ...
        <div class="col-12 col-xxl-8">
          <article class="card admin-edit-source-panel admin-edit-source-main-panel shadow-sm">
            <div class="card-body p-0 d-flex flex-column">
              ...
              <div class="p-3 p-lg-4 d-flex flex-column gap-4" v-if="showSourceEditor">
                <!-- Selector Section -->
                <div class="source-selector-section pb-3 border-bottom border-subtle">
                  <div class="mb-3">
                    <label class="form-label text-secondary small uppercase fw-bold mb-2">{{ $t('editsource.type_label') }}</label>
                    <div class="row g-2">
                      <div class="col-6 col-md-3" v-for="item in support" :key="item">
                        <button 
                          type="button" 
                          class="btn w-100 py-3 d-flex flex-column align-items-center justify-content-center gap-2 source-type-card"
                          :class="JsonSource.src === item ? 'active-source-type' : 'inactive-source-type'"
                          @click="JsonSource.src = item"
                        >
                          <i class="bi" :class="getSourceIcon(item)"></i>
                          <span>{{ $t(`editsource.type.${item}`) }}</span>
                        </button>
                      </div>
                    </div>
                  </div>

                  <div v-if="isFlatSourceSelected || JsonSource.src === 'database'" class="mt-3">
                    <label class="form-label text-secondary small uppercase fw-bold mb-2">{{ $t('editsource.format') }}</label>
                    <div class="d-flex flex-wrap gap-2">
                      <button 
                        v-for="item in (isFlatSourceSelected ? supportedFlat : supportedDb)" 
                        :key="item"
                        type="button" 
                        class="btn btn-sm rounded-pill px-3 py-1 source-format-pill"
                        :class="JsonSource.type === item ? 'active-format-pill' : 'inactive-format-pill'"
                        @click="JsonSource.type = item"
                      >
                        {{ item.toUpperCase() }}
                      </button>
                    </div>
