...
const supportedFlat = ref(["json", "yml", "csv", "xml", "hcl", "text"])
        ...
        <div class="card-header">
          <div class="row">
            <div class="col-md-2">
              ...
              <a v-if="SourceInfo" type="button" class="btn btn-primary btn-sm"
                :title="`${$t('editsource.header')} ${SourceInfo.id}`"
                :href="`${apiUrl}/data/source/${SourceInfo.id}`" target="_blank"><i class="bi bi-eye-fill"></i></a>
            </div>
            <div v-if="SourceInfo" class="col-md-9 text-center">
              <div class="input-group">
                <span class="input-group-text" id="viewname">{{ $t('editsource.header') }}</span>
                <span class="input-group-text" id="viewname">ID <i class="bi bi-arrow-right-short"></i> {{ SourceInfo.id
                  }}</span>
                <input type="text" class="form-control" placeholder="Name" aria-label="View Name"
                  aria-describedby="viewname" v-model="SourceInfo.name">
              </div>
            </div>
          </div>
        </div>
            ...
            <div class="col-md-8">
              <div class="input-group mb-3">
                ...
                <select class="form-select"
                  v-if="JsonSource.src === 'file' || JsonSource.src === 'url' || JsonSource.src === 'execute' || JsonSource.src === 'text'"
                  v-model="JsonSource.type" :class="{ 'border-success': !supportedFlat.includes(JsonSource.type) }">
                ...
                <select class="form-select" v-if="JsonSource.src === 'database'" v-model="JsonSource.type"
                  :class="{ 'border-success': !supportedDb.includes(JsonSource.type) }">
                ...
                <div class="card-body">
                  ...
                  <div class="collapse" id="collapseInfoLoop">
                    <div class="card card-body">
                      ...
                      <p>Par exemple, si vous avez une loop sur une source qui renvoie les champs <code>id</code> et
                        <code>nom</code>,
                        vous pouvez utiliser dans les différents champs <code v-text="'{{ item.id }}'"></code> pour
                        accéder à la
                        valeur de l'<code>id</code> et <code v-text="'{{ item.nom }}'"></code> pour accéder à la valeur
                        du
                        <code>nom</code>.
                      </p>
                    </div>
